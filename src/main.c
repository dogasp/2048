...
Start:;
	...
	int gridSize = 4;
	if (nb_players == 1)
		gridSize = Game_size_grid_Message(nb_players);

	int grid[2][gridSize][gridSize];
	for (int k = 0; k < 2; k++)
	{
		for (int i = 0; i < gridSize; i++)
		{ //Initialistion de la grille
	...
	{ //Début de la boucle
		...
		case SDL_KEYUP:
			...
			{
				//commandes joueur 1
				alowed = (Game_mode_3 == 0) ? (playerTurn == 0) : 1 && canPlay[0]; //si on est mode chacun son tour, on regarde si c'est à nous, alors alowed = 1
				if (event.key.keysym.sym == SDLK_RIGHT && alowed)
				{ //sinon on est mode simultané et alowed = 1 par défaut
					Save(nb_players, gridSize, Game_mode_3, grid[0], 0);
					activated = 1;
				...
				//commandes joueur 2
				alowed = (Game_mode_3 == 0) ? (playerTurn == 1) : 1 && canPlay[1] == 1;
		...
		case SDL_MOUSEBUTTONUP:
			if (mouseCanClick == 1)
			{
				if (event.button.x > Height / 2 && event.button.x < Height / 2 + Height / 10 && event.button.y > Height * 1.1 && event.button.y < Height * 1.1 + Height / 20)
				{
					int temp[8][8];
					for (int i = 0; i < 8; i++)
					{ //Initialistion de la grille
		...
		for (int i = 0; i < 2; i++)
		{
			if (canPlay[i] != 0 && moveRight(gridSize, grid[i], 0) + moveLeft(gridSize, grid[i], 0) + moveDown(gridSize, grid[i], 0) + moveUp(gridSize, grid[i], 0) == 0)
			{
				canPlay[i] = 0;
			}
		}
		...
		if (Game_mode_3 == 1)
		{ //Affichage timer en mode chrono
			...
			if (SecTimer == 300)
			{
				ended = 1;
				if (nb_players == 1)
					sprintf(endMessage, "Partie finie, ton score est de %d", SumGrid(gridSize, grid[0]));
				else
				{
					int score0 = SumGrid(gridSize, grid[0]);
					int score1 = SumGrid(gridSize, grid[1]);
